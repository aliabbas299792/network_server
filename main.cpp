...
void write_callback(event_manager *ev, processed_data write_metadata,
                    uint64_t pfd) {
  std::cout << "wrote " << write_metadata.op_res_now << " bytes of "
...
int main() {
  ...
  cbs.accept_cb = accept_callback;
  cbs.read_cb = read_callback;
  cbs.write_cb = write_callback;
  cbs.shutdown_cb = shutdown_callback;
  cbs.close_cb = close_callback;

  event_manager ev;
  ev.set_callbacks(cbs);

  auto listener_pfd = setup_listener_socket(ev, 8000);
